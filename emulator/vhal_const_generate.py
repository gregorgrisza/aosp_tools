...
if not(os.path.exists(default_vhal_location) and os.path.isdir(default_vhal_location)):
    print("Vehicle HAL was not found at %s. lunch may provide a correct environment, or files moved" % default_vhal_location)
    sys.exit(1)

# merge default types.hal with vendor specific types.hal, it's needed, because parser needs to know all types
vhal_file = os.path.join(script_directory, "temp_types.hal")

default_vhal_file = os.path.join(default_vhal_location, '2.0', 'types.hal')
vendor_vhal_file = os.path.join(vhal_location, '1.0', 'types.hal')

with open(vhal_file, 'w') as f:
    f.writelines(open(default_vhal_file, 'r').read())
    vendor_types_text = open(vendor_vhal_file, 'r').read()\
        .replace('android.hardware.automotive.vehicle@2.0::VehicleProperty', 'int32_t')\
        .replace('package vendor.v.vehicle@1.0;', '')\
        .replace('import android.hardware.automotive.vehicle@2.0;', '')\

    f.writelines(vendor_types_text)

def generateHal20(vhal_20_file):
